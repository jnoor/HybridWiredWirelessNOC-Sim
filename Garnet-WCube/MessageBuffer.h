...
public:
  ...
  NetworkMessage* peekAtHeadOfQueue() const;
  NetworkMessage* peek() const { return peekAtHeadOfQueue(); }
  //  const NetworkMessage* peekNetworkMessage() const { assert(isReady()); return m_prio_heap.peekMin().m_msgptr; }
  NetworkMessage* peekNetworkMessage() const { assert(isReady()); return m_prio_heap.peekMin().m_msgptr; }
  //  const NetworkMessage* peekNetworkMessageEvenIfNotReady() const {return m_prio_heap.peekMin().m_msgptr; }
  NetworkMessage* getNetworkMessageCopy() const;
  NetworkMessage* peekNetworkMessageEvenIfNotReady() const {return m_prio_heap.peekMin().m_msgptr; }
  void enqueue(NetworkMessage* message) { enqueue(message, 1); }
  void enqueue(NetworkMessage* message, Time delta);
  //  void enqueueAbsolute(const NetworkMessage& message, Time absolute_time);
  int dequeue_getDelayCycles(NetworkMessage* message);  // returns delay cycles of the message
  void dequeue(NetworkMessage* message);
...
private:
  // Private Methods  
  int setAndReturnDelayCycles(NetworkMessage* message);
